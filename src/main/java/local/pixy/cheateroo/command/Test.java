...
public class Test implements IClientCommandHandler {
	...
	public void onClientCommandCall(MinecraftClient mc, String message, String[] parts, String testedCommand) {
		try {
			// InputStream is =
			// mc.getResourcePackDownloader().getPack().open(ResourceType.CLIENT_RESOURCES,
			...
			URL url = DefaultResourcePack.class.getResource(
					"/" + ResourceType.CLIENT_RESOURCES.getDirectory() + "/" + id.getNamespace() + "/" + id.getPath());

			// Path path =
			// DefaultResourcePack.resourcePath.resolve(ResourceType.CLIENT_RESOURCES.getDirectory()
